...
def evaluate(Model, TT_DL, max_len, DEVICE, tokenizer):
    text_fscore = 0
    class_fscore = 0
    print('Delete Existing File')
    file_path = './result/'
    file_list = [file_path + 'text.txt', file_path + 'label.txt', file_path+ 'origin.txt']
    for f in file_list:
        os.remove(f)
    ...
    for index, content in enumerate(tqdm(TT_DL)):
        ...
        if index % 50 == 1:
            print(f'corrected_text : {text_output_list[0]}')
            print(f'o_content_text : {origin_content_list[0]}')
            print(f'label_of__text : {label_output_list[0]}')
        for i in range(0, correct_text_label.size()[0]):
            m2_output_file(prompt_decode(text_output_list[i]), label_output_list[i], origin_content_list[i], file_path + 'text.txt', file_path + 'label.txt', file_path+ 'origin.txt')
    ERRANT_evaluate(file_path)

    ### using ERRANT 

def prompt_decode(prompt_output):
    word_pieces = prompt_output.split('[')
    # word_pieces = word_pieces.split(']')
    correct_output = []
    for word_piece in word_pieces:
        try:
            if ']' in word_piece:
                prompt, origin = word_piece.split(']')
                wrong_word, correct_word = prompt.replace(' ', '').split('|')
                # print(len(correct_word)) 
                correct_output.append(correct_word.strip())
                correct_output.append(origin[1:])
            else:
                correct_output.append(word_piece[:-1])
        except:
            print(f'wrong output format : {prompt_output}')
            continue
    # print(correct_output.remove(''))
    while ('' in correct_output):
        correct_output.remove('')
...
def ERRANT_evaluate(file_path):
    ...
    Text_Org = 'errant_parallel -orig ' + file_path + 'origin.txt ' +  '-cor ' + file_path +  'text.txt ' + '-out ' + file_path + 'text_compare_origin.m2 '
    os.system(Org_Label)
    os.system(Text_Org)
    Compare_commend = 'errant_compare -hyp ' + file_path + 'text_compare_origin.m2 ' + '-ref ' + file_path + 'origin_compare_label.m2'
    ...
    result = steam.read()
    with open( file_path + 'F_score.txt', 'w+') as f:
        f.write(result)
...
if __name__ == '__main__':
    test_prompt_output = 'This is grammarical sentence. '
    print(prompt_decode(test_prompt_output))
